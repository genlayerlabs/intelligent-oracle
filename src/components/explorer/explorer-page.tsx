          ...
          <div className="flex flex-col gap-6 lg:flex-row lg:items-end lg:justify-between">
            <div>
              <div className="flex items-center gap-3">
                ...
                <div className="flex flex-wrap items-center gap-2 text-xs font-medium uppercase tracking-[0.18em] text-black/45">
                  <span>Markets</span>
                  <span>/</span>
                  <span>{ready ? "Connected" : "Configuration required"}</span>
                </div>
              </div>
              <h1 className="mt-5 text-[clamp(2.25rem,6vw,5.25rem)] font-light leading-none tracking-normal text-black">
                Market Explorer
              </h1>
              <p className="mt-4 max-w-2xl text-base font-light leading-7 text-black/65">
                Monitor registered markets, resolution status, and validator transaction details.
              </p>
            </div>
            <div className="flex flex-wrap gap-2">
              <Button asChild variant="outline" className="border-black/15 bg-transparent text-black hover:bg-black hover:text-white">
                <Link href="/assistant">
                  <Plus className="size-4" aria-hidden />
                  Create oracle
                </Link>
              </Button>
              <Button type="button" disabled={!ready || loading} onClick={() => void refreshOracles()} className="bg-black text-white hover:bg-[color:var(--brand-lavender)]">
        ...
        <section className="brand-surface mt-4 overflow-hidden rounded-md">
          <div className="grid gap-3 p-4 lg:grid-cols-[minmax(0,1fr)_auto_auto] lg:items-center">
            <div className="relative">
              <Search className="pointer-events-none absolute left-3 top-1/2 size-4 -translate-y-1/2 text-black/40" aria-hidden />
              <Input
                value={query}
                onChange={(event) => setQuery(event.currentTarget.value)}
                placeholder="Search by title, outcome, status, or address"
                className="border-black/15 bg-white/70 pl-9"
              />
            </div>
            ...
            <label className="flex items-center gap-2 text-sm text-black/55">
              ...
              <select
                ...
                className="h-9 rounded-md border border-black/15 bg-white/70 px-3 text-black outline-none focus:ring-2 focus:ring-[color:var(--brand-lavender)]/30"
              >
          ...
          {!loading && filteredOracles.length === 0 ? (
            <div className="flex min-h-72 flex-col items-center justify-center p-8 text-center">
              ...
              <h2 className="mt-4 text-sm font-medium text-black">No oracles found</h2>
          ...
          {loading && filteredOracles.length === 0 ? (
            <div className="flex min-h-72 items-center justify-center gap-2 text-sm text-black/55">
              <Spinner />
              Loading markets
            </div>
          ) : null}
          ...
          {filteredOracles.length > 0 ? (
            <div className="overflow-x-auto">
              <table className="hidden min-w-full border-separate border-spacing-0 text-sm md:table">
                <thead className="bg-white/45 text-left text-xs uppercase tracking-[0.14em] text-black/45">
                  <tr>
                    <th className="border-b border-black/10 px-4 py-3 font-medium">Market</th>
                    <th className="border-b border-black/10 px-4 py-3 font-medium">Status</th>
                    <th className="border-b border-black/10 px-4 py-3 font-medium">Outcome</th>
                    <th className="border-b border-black/10 px-4 py-3 font-medium">Earliest resolution</th>
                    <th className="border-b border-black/10 px-4 py-3 font-medium">Address</th>
                  </tr>
                </thead>
                <tbody>
                    ...
                    <tr key={oracle.address} className="group">
                      <td className="border-b border-black/10 px-4 py-4">
                        <Link href={`/oracle/${oracle.address}`} className="block">
                          <p className="font-medium text-black group-hover:text-[color:var(--brand-lavender)]">
                            {oracle.title || "Untitled oracle"}
                          </p>
                          <p className="mt-1 line-clamp-1 max-w-xl text-black/55">
                      ...
                      <td className="border-b border-black/10 px-4 py-4 text-black/55">
                        {oracle.outcome || "Pending"}
                      </td>
                      <td className="border-b border-black/10 px-4 py-4 text-black/55">
                        {formatDate(oracle.earliest_resolution_date)}
                      </td>
                      <td className="border-b border-black/10 px-4 py-4 font-mono text-xs text-black/50">
                ...
                {filteredOracles.map((oracle) => (
                  <Link key={oracle.address} href={`/oracle/${oracle.address}`} className="block p-4">
                    <div className="flex items-start justify-between gap-3">
                      <div>
                        <h2 className="font-medium text-black">{oracle.title || "Untitled oracle"}</h2>
...
function Metric({ label, value }: { label: string; value: number }) {
  return (
    <div className="brand-surface-solid rounded-md p-4">
      ...
      <p className="mt-3 text-3xl font-light leading-none text-black">{value}</p>
    </div>
  );
}
